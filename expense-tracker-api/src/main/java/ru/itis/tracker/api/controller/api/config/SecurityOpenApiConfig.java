package ru.itis.tracker.api.controller.api.config;
...
public class SecurityOpenApiConfig {
    ...
    private Info buildInfo() {
        return new Info().title("Expense Tracker API");
    }
    ...
    private Components buildComponents() {
        ...
        Schema<?> unauthorizedErrorSchema = new Schema<>()
                .type("object")
                .addProperty("timestamp", new Schema<>().type("integer").format("int32")
                        .example(1677759133))
                .addProperty("error", new Schema<>().type("string").example("Unauthorized"))
                .addProperty("status", new Schema<>().type("integer").example(401))
                .addProperty("path", new Schema<>().type("string").example("/auth/token"));
